...
class DomainTestCase(BaseTestCase):
    ...
    def test_create_roadmap(self):
        self.login()
        response = self.post('/v1/domain/roadmap', json=dict(
            title = "Python入门方案",
            description = "适合0基础的孩子",
            heads = [2]
        ))
        self.OK(response)
        response = self.get('/v1/domain/roadmap', query_string=dict(
            roadmap = 1
        ))
        data = response.get_json()
    ...
    def test_put_roadmap(self):
        ...
        response = self.put('/v1/domain/roadmap', json=dict(
            title = "Python基础",
            roadmap = 1
        ))
        response = self.get('/v1/domain/roadmap', query_string=dict(
    ...
    def test_learn_roadmap(self):
        ...
        response = self.post('/v1/domain/roadmap/learn', query_string=dict(
            roadmap = 1
        ))
        self.OK(response)
        response = self.delete('/v1/domain/roadmap/learn', query_string=dict(
            roadmap = 1
        ))
        self.OK(response)